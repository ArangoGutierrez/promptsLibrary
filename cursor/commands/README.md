...
```
┌─────────────────┐
│ Cursor (Opus)   │ /architect "Add caching"
│ - Architecture  │ [Explore, prototype, decide]
...
└────────┬────────┘
  ...
  .plans/plan-*.md
         ...
         ▼
┌─────────────────┐
│ Terminal        │ claude code .plans/plan-*.md
│ (Sonnet)        │ [Implement, test, commit]
│ - Implementation│
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ Cursor (Opus)   │ /review-cli-work
│ - Review        │ /git-polish → /push
│ - Polish        │
└─────────────────┘
```

**Benefits**:
- ✅ Best model for each phase (Opus plans, Sonnet codes)
- ✅ 60% cost reduction on implementation
- ✅ Faster iteration in terminal
- ✅ Full review cycle in Cursor

### 📋 Traditional Task Workflow
...
[Full pipeline: explore → challenge → prototype → synthesize]
...
```
.plans/
  ├── plan-arch-{timestamp}.md      # Architecture decisions
  ├── plan-task-{timestamp}.md      # Task specifications
  └── plan-refactor-{timestamp}.md  # Refactor plans
```
...
```
.prototypes/
  ├── approach-1/    # First prototype
  ├── approach-2/    # Second prototype
  └── approach-3/    # Third prototype (if --prototype 3)
```
...
**Feature development**:
```
/architect → /export-plan → [terminal] → /review-cli-work → /push
```

**Bug fix**:
```
/debug → /test → /self-review → /push
```

**Refactor**:
```
/refactor → /export-plan → [terminal] → /review-cli-work → /audit → /push
```

**Research spike**:
```
/research → /architect --quick → /task "Implement findings"
```
