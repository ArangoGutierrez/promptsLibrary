...
- **Devil's Advocate (DA)** — used by `dispatch-da.sh` via the NVIDIA
  inference API (Nemotron-3 super). Includes a one-shot example because
  reasoning models benefit from concrete format demonstration.
...
Dispatched via: `dispatch-da.sh` → NVIDIA inference API → Nemotron-3 super.
...
- Model: configurable via `$CLAUDE_PANEL_DA_MODEL` (default
