...
description: Validate (Recommended) options in AskUserQuestion via two parallel panelists — devil's advocate (Nemotron) + principal-engineer (Claude subagent). Triggered by the validate-recommendation hook; do not invoke manually.
---
...
- **`Bash`** tool:
  ...
  ```
  Note: `dispatch-da.sh` reads `$NVIDIA_INFERENCE_API_KEY` from the
  inherited environment. Do not pass it on the command line; do not
  echo it anywhere.
...
The recommendation stands. For this question, take the recommended
option without asking the user. Print a brief one-paragraph message
so the user can interrupt if surprised:

> Panel validated `<recommended_label>` for "<question excerpt>".
> DA (Nemotron): <abbreviated rationale from aggregate stdout>
> PE (subagent): <abbreviated rationale from aggregate stdout>
> Proceeding.
...
Something went wrong (Nemotron API down, malformed PE response,
...
| `$NVIDIA_INFERENCE_API_KEY` unset | dispatch-da.sh writes an ERROR verdict; aggregate emits ERROR; follow ERROR branch. The user-facing message ends up the same. |
...
- Do NOT echo `$NVIDIA_INFERENCE_API_KEY` in any user-facing text or
  trace output.
- Do NOT write the API key to any file.
- Do NOT fall back to `reasoning_content` from Nemotron responses if
  `content` is null — `dispatch-da.sh` is responsible for that
  decision and intentionally emits ERROR in that case.
