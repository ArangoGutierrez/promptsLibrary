...
- **NAT workflow YAML mode**: NAT also supports `nat run --config_file workflow.yml` as a standalone CLI. We're using the Python library API instead (in-process). If we later want to make the panel workflow externally invocable via `nat run` (e.g., from a non-Claude context), we'd add a panel-workflow.yml file. Out of scope for v2.1.
