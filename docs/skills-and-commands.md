...
| `test-driven-development` | Enforces the RED → GREEN → REFACTOR cycle with explicit phase signals; blocks implementation without a failing test | During any implementation work. This skill is what drives TDD discipline — there is no TDD hook; it pairs with the local `/tdd-protocol` skill |
...
**The same rules are enforced in both tools.** Signed commits (`sign-commits.sh`) and worktree isolation (`enforce-worktree.sh`) run as Claude Code hooks, while TDD discipline is skill-driven (the `test-driven-development` and `/tdd-protocol` skills, plus the constitution's theater-test rules) rather than hook-enforced. The Cursor commands independently enforce these same rules in their own flows: `/git-polish` requires `-S -s` flags, `/worktree` always branches from the remote ref, `/code` commits atomically with `-s -S`.
